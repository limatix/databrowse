...
###############################################################################
""" plugins/handlers/dbh_directory_iamge.py - Image Directory Handler """
...
def dbh_directory_image(path, contenttype, extension):
    ...
    if contenttype.startswith("inode/directory") or contenttype.startswith("application/x-directory") or contenttype.startswith("directory"):
        ...
        count = 0
        for item in dirlist:
            if (os.path.splitext(item)[1].lower() in [".png", ".jpg", ".jpeg", ".gif", ".bmp", ".tif", ".tiff"]):
                count = count + 1
        if count > (len(dirlist) * 0.5):
            return "db_directory_image"
        else:
            return False
    else:
        return False
